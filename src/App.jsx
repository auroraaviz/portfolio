...
// Modos manuales — los 4 más especiales visualmente
const SKY_MODES = {
...
// Detecta el modo inicial según la hora para que al entrar se vea algo coherente
...
const generateClouds = (count, yMin, yMax, sizeMin, sizeMax, speedMin, speedMax) =>
    Array.from({ length: count }, (_, i) => {
        ...
        return {
            ...
            base, blobs: makeBlobs(base, i * 0.7), duration,
            delay:   -(Math.random() * duration),
...
function Nebulae() {
    return (
        <>
            <div style={{ position: "absolute", top: "8%", left: "5%", width: "35%", height: "28%",
            ...
            <div style={{ position: "absolute", top: "35%", right: "8%", width: "28%", height: "22%",
            ...
            <div style={{ position: "absolute", bottom: "15%", left: "30%", width: "40%", height: "20%",
...
function SkySelector({ currentMode, onSelect }) {
    ...
    return (
        <div style={{
            ...
            {/* Botón principal — swatch del modo actual */}
            <motion.button
                onClick={() => setExpanded(e => !e)}
                whileHover={{ scale: 1.08 }}
                whileTap={{ scale: 0.95 }}
                title="Cambiar modo de cielo"
                style={{
                    width: 48, height: 48, borderRadius: "50%",
                    background: `linear-gradient(to bottom, ${SKY_MODES[currentMode].swatch[0]}, ${SKY_MODES[currentMode].swatch[2]})`,
                    ...
                    >
                        {MODE_KEYS.map(key => {
                            ...
                            return (
                                <motion.button
                                    key={key}
                                    onClick={() => { onSelect(key); setExpanded(false); }}
                                    whileHover={{ x: 3 }}
                                    style={{
                                ...
                                >
                                    ...
                                    <div style={{
                                        ...
                                        background: `linear-gradient(to bottom, ${mode.swatch[0]}, ${mode.swatch[2]})`,
                                        border: isActive ? "2px solid rgba(233,213,255,0.7)" : "1.5px solid rgba(167,139,250,0.3)",
                                        boxShadow: isActive ? "0 0 8px rgba(167,139,250,0.5)" : "none",
                                    }} />
                                    {/* Label */}
                                    <span style={{
                                        ...
                                        color: isActive ? "#e9d5ff" : "rgba(196,165,253,0.65)",
                                        whiteSpace: "nowrap",
                                    }}>
                                        {mode.icon} {key}
                                    </span>
                    ...
                    <div style={{ position: "relative", width: cloudW, height: cloud.base * 0.9 }}>
                        {cloud.blobs.map((blob, bi) => (
                            <div key={bi} style={{
                                ...
                                borderRadius: "50%",
                                background: bi === 0 ? "rgba(248,240,255,0.92)"
                                    : bi % 3 === 0 ? "rgba(230,215,255,0.78)"
                ...
                {!isDaytime && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 0.5 }} transition={{ duration: 2 }}
                        style={{
                            ...
                            filter: "blur(8px)", zIndex: 1, pointerEvents: "none",
                        }}
                    />
                )}

                {/* Estrellas — visibles en todos los modos oscuros, tenues en amanecer/atardecer */}
                {(sky.label === "noche" || sky.label === "anochecer" || sky.label === "amanecer" || sky.label === "atardecer") &&
                    stars.map((star) => {
                        // Opacidad máxima según el modo
                        const modeMult = sky.label === "noche" || sky.label === "anochecer" ? 1
                            : sky.label === "atardecer" ? 0.35   // pocas, las primeras que aparecen
                            : 0.2;                                // amanecer: últimas en desvanecerse

                        // En amanecer/atardecer solo mostramos estrellas "near" y algunas "mid"
                        ...
                        return (
                            <motion.div key={star.id}
                                animate={{
                                    opacity: [opacityMax * 0.5, opacityMax, opacityMax * 0.5],
                                ...
                                style={{
                                    ...
                                    boxShadow: isNight && star.layer === "near"
                                        ? `0 0 ${star.size * 3}px ${star.size}px rgba(196,165,253,0.55)`
                                        : isNight && star.layer === "mid"
